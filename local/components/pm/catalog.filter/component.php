...
if ($this->StartResultCache(false, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups())))
{
	...
	if (in_array("SECTION_ID", $arParams["FIELD_CODE"]))
	{
		$rsSection = CIBlockSection::GetList(
			...
			false,
			Array("ID", "DEPTH_LEVEL", "NAME", "CODE")
		);
		while($arSection = $rsSection->Fetch())
		{
			$arResult["arrSection"][$arSection["ID"]] = array(
				"ID" => $arSection["ID"],
				"NAME" => $arSection["NAME"],
                "CODE" => $arSection["CODE"],
				"ITEMS" => array()
			...
			$rsSection1 = CIBlockSection::GetList(
				Array("left_margin"=>"asc"),
				Array(
					"IBLOCK_ID"=>$arParams["IBLOCK_ID"],
				...
				false,
				Array("ID", "DEPTH_LEVEL", "NAME", "SECTION_PAGE_URL")
			);
			while($arSection1 = $rsSection1->GetNext())
			{
				$arSection1["SELECTED"] = $arParams["PATH"] == $arSection1["SECTION_PAGE_URL"];
				//$arSection1["VALUE"] = $arSection1["SECTION_PAGE_URL"];
				//if (isset($arParams["TYPE"]) && $arParams["TYPE"] == "opt") {
					$arSection1["VALUE"] = $arSection1["ID"];
				//}
				$arResult["arrSection"][$arSection["ID"]]["ITEMS"][$arSection1["ID"]] = $arSection1;
			}
		}
	}
	...
	while ($arProp = $rsProp->Fetch())
	{
		if(in_array($arProp["CODE"],$arParams["PROPERTY_CODE"]) && $arProp["PROPERTY_TYPE"] != "F")
		{
			...
			if ($arProp["PROPERTY_TYPE"]=="L")
			{
				...
				$arrEnum = array();
				$rsEnum = CIBlockProperty::GetPropertyEnum($arProp["ID"], array("VALUE" => "ASC"));
				...
				$arTemp["PROPERTY_TYPE"] = "L";
				$arTemp["MULTIPLE"] = "Y";

				$arrEnum = array();

				$rsEnum = CIBlockElement::GetList(array(), array(
					"IBLOCK_ID" => $arParams["IBLOCK_ID"],
				), array("PROPERTY_" . $arProp["CODE"]), false);
				while($arEnum = $rsEnum->Fetch()) {
					$arEnum["VALUE"] = $arEnum["ID"] = $arEnum["PROPERTY_" . ToUpper($arProp["CODE"]) . "_VALUE"];

					if (!trim($arEnum["VALUE"])) continue;
...
foreach($arParams["FIELD_CODE"] as $field_code)
{
	...
	switch ($field_code)
	{
		...
		case "SHOW_COUNTER":
			...
			break;
		case "SECTION_ID":
			$field_res = '<select id="section-path" name="'.$name.'[]" multiple data-title="Категория">';
			$field_res .= '<option value="/">'.GetMessage("CC_BCF_TOP_LEVEL").'</option>';

			foreach ($arResult["arrSection"] as $ids => $arGroup) {
				//$field_res .= '<optgroup value="/category/'.$arResult["arrSection"][$ids]["CODE"].'/" label="'.$arGroup["NAME"].'">';
				$field_res .= '<option class="optgroup" value="'.$arGroup["ID"].'">'.$arGroup["NAME"].'</option>';
				foreach ($arGroup["ITEMS"] as $sID => $arSection) {
					$checked = $arSection["SELECTED"] || in_array($arSection["ID"], (array)$value) ? ' selected' : '';
					$field_res .= '<option value="'.$arSection["VALUE"].'"'.$checked.'>'.$arSection["NAME"].'</option>';
				}
				$field_res .= '</optgroup>';
			}
			$field_res .= '</select>';
			...
			$field_res .= "<br>".InputType("hidden", $_name, "Y", $_value, false, "", "");//."&nbsp;".GetMessage("CC_BCF_INCLUDE_SUBSECTIONS");
		...
		case "L":
			...
			else
			{
				...
				if ($arProp["MULTIPLE"]=="Y") {
                    if($arProp['NAME'] == 'Месторождение')
                        $arProp['NAME'] = 'Страна происхождения';
					$res .= '<select id="prop' . $prop_id . '" multiple name="' . $name . '[]" size="' . $arParams["LIST_HEIGHT"] . '" data-title="' . $arProp["NAME"] . '">';
				} else {
					$res .= '<select id="prop'.$prop_id.'" name="'.$name.'">';
					$res .= '<option value="">'.$arProp["NAME"]/*.GetMessage("CC_BCF_ALL")*/.'</option>';
				}
				$list[""] = $arProp["NAME"];//GetMessage("CC_BCF_ALL");

				$arGroup = array();
				foreach ($arProp["VALUE_LIST"] as $key => $val) {
					$arGroup[ToUpper($val[0])][$key] = $val;
				}

				foreach ($arGroup as $char => $_arGroup) {
					$res .= '<optgroup label="' . $char . '">';

					foreach($_arGroup as $key=>$val)
					{
						$res .= '<option';
...
{
	$res_price = '<div id="slider-price"></div><div id="slider-result" class="prop-value">';
	$arResult["arrInputNames"][$FILTER_NAME."_cf"]=true;

	$min_value = pmcache('getCatalogMinPrice', $arParams["IBLOCK_ID"], $arPrice["ID"], $arParams["SECTION_CODE"]);
	$max_value = pmcache('getCatalogMaxPrice', $arParams["IBLOCK_ID"], $arPrice["ID"], $arParams["SECTION_CODE"]);
	...
	$res_price .= 'от&nbsp;<input id="price_l" type="text" name="'.$name_left.'" size="'.($arParams["NUMBER_WIDTH"]).'" data-min="'.$min_value.'" value="'.htmlspecialcharsbx($value_left).'">&nbsp;';
	...
	$res_price .= 'до&nbsp;<input id="price_r" type="text" name="'.$name_right.'" size="'.$arParams["NUMBER_WIDTH"].'" data-max="'.$max_value.'" value="'.htmlspecialcharsbx($value_right).'">';
